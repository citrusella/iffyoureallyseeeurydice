...
- TTABs are an old version and need to be upgraded. There may be more than one way to do this. (Opening and saving a prototype TTAB in IffPencil will *not* correctly upgrade it (namely, the game may treat its pie menu strings weirdly, and it might still crash other editors that don't handle prototype TTAB resources well), so don't do it that way--though you could manually make an identical TTAB (and shift IDs and such around, of course) that *should* work using IffPencil.)
...
- NAME chunk (1.0 iff label format) handling was implemented in a shortcut sort of way. I don't anticipate this being a problem for the vast majority of objects, because most objects only have one NAME chunk, or the NAME with all the info is the last one in the file.  However, if you find you get a file with *several* unexpected "not found" labels, please file an issue for me to look into it. That way, I can determine if this shortcut caused it and how best to implement a new way of handling, if needed. (I am [looking into changing NAME handling anyway](https://github.com/citrusella/iffyoureallyseeeurydice/issues/2), but if a pressing issue arises it may be treated with more urgency.)
