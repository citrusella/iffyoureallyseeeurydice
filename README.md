...
- I want to implement a system that will allow the application to remember the most recently selected folder, at least for the current session (that is, time since the application was opened last). Currently, it defaults to the current user account's downloads folder and never deviates from this location on initial open of the file dialog.
