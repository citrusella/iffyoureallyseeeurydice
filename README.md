...
- You may rarely need to double-check that the OBJD defines the right number of DGRPs for the given object or tile, as some objects have an incorrect number in this field.
- TTABs are an old version and need to be upgraded. There may be more than one way to do this. (Opening and saving a prototype TTAB in IffPencil will *not always* correctly upgrade it (namely, the game may treat its pie menu strings weirdly, and it might still crash other editors that don't handle prototype TTAB resources well), so don't do it that way. You may be able to manually make an identical TTAB (and shift IDs and such around, of course) that *should* work using IffPencil.)
